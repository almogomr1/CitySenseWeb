...
import { TeamMember, TeamType, TeamUpdateRequest } from "../redux/api/types";
...
const TeamUpdate: React.FC = () => {
  ...
  const [selectedMembers, setSelectedMembers] = useState<SelectOptionType[]>([]);
  ...
  useEffect(() => {
    if (teamData) {
      ...
      const mappedMembers = teamData.members?.map((member: TeamMember) => ({
        value: member._id,
        label: member.fullname,
      })) || [];
      setSelectedMembers(mappedMembers);
      setValue("members", mappedMembers.map((m: any) => m.value));
    }
  }, [teamData, setValue]);
              ...
              <Col md={6}>
                <FormGroup>
                  ...
                  <Select
                    ...
                    className={`react-select ${classnames({ 'is-invalid': errors.members })}`}
                    onChange={(selected: any) => {
                      setSelectedMembers(selected);
                      setValue("members", selected?.map((member: any) => member.value) || []);
                    }}
                  />
                  {Object.keys(errors).length && errors.members ? (
                      <small className="text-danger mt-1">You must select at least one member</small>
                    ) : null}
                </FormGroup>
              </Col>
            </Row>
